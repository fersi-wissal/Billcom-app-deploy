...
public class TeamService {
	...
	public List<Team> teamListOfLeader() {
		return teamRepository.findAll().stream().filter(team -> team.getLeader().getUser().getId() == getCurrentUser().getId())
	...
	public List<CountDto> taskDoneTeamMeber() {
		...
		for (TeamDto t : teamOfTeamMember()) {
				if (t.getTeamMember().getUser().getId() == user.getId() & ! taskRepository.findAllByTeamMember(t.getTeamMember()).isEmpty()) {
